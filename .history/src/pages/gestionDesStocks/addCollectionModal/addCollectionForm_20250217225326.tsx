...
const AddCollectionForm: FC<AddCollectionFormInt> = ({
  ...
  const handleSubmit = async () => {
    ...
    // try {
    //   const result =
    //     await window.electronAPI.createCollection(addCollectionData);
    //   // alert(result);
    //   console.log(result);
    // } catch (error) {
...
```
```typescript
// filepath: /home/simo/MEGA/projects/restaurentStockManag/src/electron/preload.ts
import { contextBridge, ipcRenderer } from 'electron';
import { createCollectionData } from './interfaces/requests.js';

contextBridge.exposeInMainWorld('electronAPI', {
  createCollection: (data: createCollectionData) =>
    ipcRenderer.invoke('createCollection', data),
});
